...
//				System.out.println("The number of rows = " + res.size());
	...
	public ArrayList<MovieData> getMovieInfoForSearch(String sval, String tbl) {
		...
		String sql = "select * from " + tbl
				+ " f join film_actor fa on f.film_id=fa.film_id join actor_info ai on fa.actor_id=ai.actor_id where f.title like '%"
				+ sval + "%' or f.description like '%" + sval + "%' or ai.first_name like '%" + sval
				+ "%' or ai.last_name like '%" + sval + "%';";
